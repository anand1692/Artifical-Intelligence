...
(define (tell premise)
	(cond ((null? premise) '(EMPTY PREMISE))
		  ...
		  ((equal? (list (car premise)) '(AND)) (let* ((and_var1 (operator_and (list (car (cdr premise)))))
													   ...
													   (set! kb (append and_var1 kb))
													   (set! kb (append and_var2 kb))))); (display kb) (newline))))
														   	
	(set! kb (dedupe_kb kb)))

(define (resolve_item item plist rlist)
	(if (null? plist) rlist
		(let ((resolution (resolve item (car plist))))
		;	 (display '(resolution :)) (display resolution) (newline)
			 (if (equal? resolution #f) (resolve_item item (cdr plist) rlist)
				 (resolve_item item (cdr plist) (append (list resolution) rlist))))))

; mlist =  main list with elements to resolve, plist = list of premises with whom to resolve, rlist = new resolution premises   
(define (resolve_prem mlist plist rlist)
	(cond ((null? plist) '(UNKNOWN))
		  ((member '(CONTRADICTION) rlist) '(CONTRADICTION))
		  ((null? mlist) (resolve_prem (dedupe_kb (append mlist plist)) rlist '()))
		  (else
			(let ((new_rlist (resolve_item (car mlist) plist rlist)))
				 (resolve_prem (cdr mlist) plist (dedupe_kb new_rlist))))))  

(define (ask premise)
	(cond ((null? premise) '())
		  ((not (list? premise)) (let* ((negative (operator_not (list premise)))
			   						   (new_kb (append (list negative) kb))
			   						   (result (resolve_prem new_kb new_kb '())))
			   						   (if (equal? result '(CONTRADICTION)) #t
				   						   '(UNKNOWN))))
		  ((equal? (list (car premise)) '(BICONDITIONAL)) (let* ((negative (operator_not (list premise)))
															     (new_kb1 (append (list (car negative)) kb))
																 (new_kb2 (append (list (car (cdr negative))) kb))
																 (result1 (resolve_prem new_kb1 new_kb1 '())))
																 (if (equal? result1 '(CONTRADICTION)) (let ((result2 (resolve_prem new_kb2 new_kb2 '())))
